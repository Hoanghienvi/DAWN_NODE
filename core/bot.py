...
class Bot(DawnExtensionAPI):
    ...
    async def process_reverify_email(self, link_sent: bool = False, retries = 3) -> OperationResult:
        task_id = None
            ...
            match error.error_type:
                ...
                case APIErrorType.CAPTCHA_EXPIRED:
                    ...
                    if retries > 0:
                        ...
                        return await self.process_reverify_email(link_sent=link_sent, retries = retries)
    ...
    async def login_new_account(self):
        ...
        try:
            ...
            match error.error_type:
                case APIErrorType.INCORRECT_CAPTCHA:
                    logger.warning(f"Account: {self.account_data.email} | {Fore.YELLOW}Captcha answer incorrect, re-solving...{Style.RESET_ALL}")
                    if task_id:
                        await captcha_solver.report_bad(task_id)
                ...
                case APIErrorType.CAPTCHA_EXPIRED:
                    logger.warning(f"Account: {self.account_data.email} | {Fore.YELLOW}Captcha expired, re-solving...{Style.RESET_ALL}")
                    return await self.login_new_account()
    ...
    async def handle_existing_account(self, db_account_data, verify_sleep: bool = True) -> bool | None:
        ...
        if not status:
            logger.warning(
                f"Account: {self.account_data.email} | {Fore.YELLOW}Session is invalid, re-logging in: {result}{Style.RESET_ALL}"
            )
            await self.clear_account_and_session()
            return await self.process_farming()

        logger.info(f"Account: {self.account_data.email} | {Fore.GREEN}Using existing session{Style.RESET_ALL}")
        return True

    async def handle_session_blocked(self):
        await self.clear_account_and_session()
        logger.error(
            f"Account: {self.account_data.email} | {Fore.YELLOW}Session rate-limited | Sleeping...{Style.RESET_ALL}"
        )
    ...
    async def perform_farming_actions(self):
        try:
            ...
            logger.success(
                f"{Fore.GREEN}Account: {self.account_data.email} | Sent keepalive request {Style.RESET_ALL}"
            )

            user_info = await self.user_info()
            logger.info(
                f"Account: {self.account_data.email} |{Fore.GREEN} Total points earned: {user_info['rewardPoint']['points']}{Style.RESET_ALL}"
            )
